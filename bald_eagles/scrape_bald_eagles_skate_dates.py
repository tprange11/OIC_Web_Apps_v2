'''Standalone script (run by cron) that pulls Bald Eagles skate dates from the Schedule Werks
calendar, adds new ones to BaldEaglesSkateDate and emails subscribed users.'''
...
def get_schedule_data(from_date, to_date):
    ...
    for item in data:
        if "Baldies" in item["text"]: # Schedule Werks labels the Bald Eagles skate "Baldies"
...
def add_skate_dates(sessions):
    '''Adds Bald Eagles dates and times to the BaldEaglesSkateDate model.
...
def send_skate_dates_email():
    ...
    for recipient in recipients:
        if recipient.user.is_active:
            ...
            text_message += f'https://www.oicwebapp.com/web_apps/bald_eagles/\n\n'
...
if __name__ == "__main__":
    ...
    # Every Saturday request tomorrow's (Sunday's) schedule and parse it for Bald Eagles dates
