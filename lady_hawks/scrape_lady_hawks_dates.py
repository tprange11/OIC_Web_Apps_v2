...
from lady_hawks.models import LadyHawksSkateDate
from accounts.models import Profile

skate_dates = []

def scrape_oic_schedule(date):
    '''Scrapes Ozaukee Ice Center schedule website for Lady Hawks skate dates.'''
    ...
    for row in rows:
        ...
        if len(cols) > 2:
            if cols[4].get_text().strip() == "Lady Hawks":
...
def send_skate_dates_email():
    '''Sends email to Users letting them know when Lady Hawks skate dates are added.'''
    recipients = Profile.objects.filter(lady_hawks_email=True).select_related('user')

    for recipient in recipients:
        if recipient.user.is_active:
            to_email = [recipient.user.email]
            from_email = 'no-reply@mg.oicwebapps.com'
            subject = 'New Lady Hawks Skate Date Added'

            # Build the plain text message
            text_message = f'Hi {recipient.user.first_name},\n\n'
            text_message += f'New Lady Hawks skate dates are now available online. Sign up at the url below.\n\n'
            text_message += f'https://www.oicwebapps.com/web_apps/lady_hawks/\n\n'
            ...
            html_message = render_to_string(
                'lady_hawks_skate_dates_email.html',
                {
...
if __name__ == "__main__":
    ...
    # Every Saturday scrape the next Tuesday and Friday for Lady Hawks skate dates
