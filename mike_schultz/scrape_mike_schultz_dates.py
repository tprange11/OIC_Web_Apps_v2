...
from mike_schultz.models import MikeSchultzSkateDate
...
def scrape_oic_schedule(date):
    ...
    for row in rows:
        ...
        if len(cols) > 2:
            if cols[4].get_text().strip() == "Mike Schultz":
...
def send_skate_dates_email():
    '''Sends email to Users letting them know when Mike Schultz skate dates are added.'''
    recipients = Profile.objects.filter(mike_schultz_email=True).select_related('user')

    for recipient in recipients:
        if recipient.user.is_active:
            to_email = [recipient.user.email]
            from_email = 'no-reply@mg.oicwebapps.com'
            subject = 'New Mike Schultz Skate Date Added'

            # Build the plain text message
            text_message = f'Hi {recipient.user.first_name},\n\n'
            text_message += f'New Mike Schultz skate dates are now available online. Sign up at the url below.\n\n'
            text_message += f'https://www.oicwebapps.com/web_apps/mike_schultz/\n\n'
            ...
            html_message = render_to_string(
                'mike_schultz_skate_dates_email.html',
                {
