...
class KranichSkateDate(models.Model):
    '''Model holds dates for the Kranich Skate.'''
    ...
    def registered_skaters(skate_date):
        ...
        num_goalies = KranichSkateSession.objects.filter(skate_date=skate_date, goalie=True).count()
        num_skaters = KranichSkateSession.objects.filter(skate_date=skate_date, goalie=False).count()
        return {'num_skaters': num_skaters, 'num_goalies': num_goalies}


class KranichSkateSession(models.Model):
    '''Model that stores skate session data.'''

    # Model Fields
    skater = models.ForeignKey(User, on_delete=models.CASCADE)
    skate_date = models.ForeignKey(KranichSkateDate, on_delete=models.CASCADE, related_name='session_skaters')
