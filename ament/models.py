...
class AmentSkateDate(models.Model):
    '''Model holds dates for the Ament Skate.'''
    ...
    def registered_skaters(skate_date):
        ...
        num_goalies = AmentSkateSession.objects.filter(skate_date=skate_date, goalie=True).count()
        num_skaters = AmentSkateSession.objects.filter(skate_date=skate_date, goalie=False).count()
        return {'num_skaters': num_skaters, 'num_goalies': num_goalies}


class AmentSkateSession(models.Model):
    '''Model that stores skate session data.'''

    # Model Fields
    skater = models.ForeignKey(User, on_delete=models.CASCADE)
    skate_date = models.ForeignKey(AmentSkateDate, on_delete=models.CASCADE, related_name='session_skaters')
