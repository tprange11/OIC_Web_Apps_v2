'''Standalone script (run by cron) that pulls Ament Skate dates from the Schedule Werks
calendar, adds new ones to AmentSkateDate and emails subscribed users.'''
...
from ament.models import AmentSkateDate
...
def add_skate_dates(sessions):
    '''Adds Ament Skate dates and times to the AmentSkateDate model.
...
def send_skate_dates_email():
    '''Sends email to Users letting them know when Ament Skate dates are added.'''
    recipients = Profile.objects.filter(ament_email=True).select_related('user')

    for recipient in recipients:
        if recipient.user.is_active:
            to_email = [recipient.user.email]
            from_email = 'no-reply@mg.oicwebapp.com'
            subject = 'New Ament Skate Date Added'

            # Build the plain text message
            text_message = f'Hi {recipient.user.first_name},\n\n'
            text_message += f'New Ament Skate dates are now available online. Sign up at the url below.\n\n'
            text_message += f'https://www.oicwebapp.com/web_apps/ament/\n\n'
...
if __name__ == "__main__":
    ...
    # Every Monday request the schedule six days out and parse it for Ament Skate dates
