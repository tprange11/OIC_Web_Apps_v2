...
class WomensHockeySkateDateListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        skate_sessions = self.session_model.objects.filter(
            skate_date__skate_date__gte=date.today()).order_by('pk')
...
class DeleteWomensHockeySkateSessionView(SessionRemovalMixin, DeleteView):
    ...
    owner_field = 'user'
    program_filter = {'pk': 8}
    cart_item_name = 'Womens Hockey'   # hard-coded in CreateWomensHockeySkateSessionView.add_to_cart
    removed_message = 'Skater has been removed from that skate session!'

# The following views are for staff only.


class WomensHockeySkateDateStaffListView(LoginRequiredMixin, StaffRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        session_skaters = self.sessions_model.objects.filter(
            skate_date__skate_date__gte=date.today()).order_by('skate_date', 'pk')
