...
from womens_hockey.models import WomensHockeySkateDate
...
def scrape_oic_schedule(date):
    '''Scrapes Ozaukee Ice Center schedule website for Womens Hockey skate dates.'''
    ...
    browser.open(
        "https://ozaukeeicecenter.maxgalaxy.net/ScheduleList.aspx?ID=2")
    ...
    browser["ctl00_ContentPlaceHolder1_txtFromDate_dateInput_ClientState"] = '{"enabled":true,"emptyMessage":"","validationText":"'+today_with_time + \
        '","valueAsString":"'+today_with_time + \
        '","minDateStr":"1980-01-01-00-00-00","maxDateStr":"2099-12-31-00-00-00","lastSetTextBoxValue":"'+xx_xx_xxxx+'"}'
    browser["ctl00_ContentPlaceHolder1_txtThroughDate_dateInput_ClientState"] = '{"enabled":true,"emptyMessage":"","validationText":"'+today_with_time + \
        '","valueAsString":"'+today_with_time + \
        '","minDateStr":"1980-01-01-00-00-00","maxDateStr":"2099-12-31-00-00-00","lastSetTextBoxValue":"'+xx_xx_xxxx+'"}'
    ...
    for row in rows:
        ...
        if len(cols) > 2:
            if cols[4].get_text().strip() == "Womens Open Hockey":
                skate_dates.append(
                    [date, cols[0].get_text().strip(), cols[1].get_text().strip()])
                break
...
def add_skate_dates(sessions):
    ...
    for session in sessions:
        try:
            data = model(
                skate_date=session[0], start_time=session[1], end_time=session[2])
            data.save()
            new_dates = True
...
def send_skate_dates_email():
    '''Sends email to Users letting them know when Womens Hockey skate dates are added.'''
    recipients = Profile.objects.filter(
        womens_hockey_email=True).select_related('user')

    for recipient in recipients:
        to_email = [recipient.user.email]
        from_email = 'no-reply@mg.oicwebapps.com'
        subject = 'New Womens Hockey Skate Date Added'

        # Build the plain text message
        text_message = f'Hi {recipient.user.first_name},\n\n'
        text_message += f'New Womens Hockey skate dates are now available online. Sign up at the url below.\n\n'
        text_message += f'https://www.oicwebapps.com/web_apps/womens_hockey/\n\n'
        ...
        html_message = render_to_string(
            'womens_hockey_skate_dates_email.html',
            {
...
if __name__ == "__main__":
    ...
    # Every day scrape the next four weeks for Sunday Womens Hockey skate dates
    for x in range(28):
